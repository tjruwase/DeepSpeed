...
static void _safe_handle_register(const int fd,
...
static void _safe_buffer_register(void* gpu_buffer,
                                  const size_t size)
{
                  ...
                  validate)
{
    ...
    _safe_handle_register(fd,
                          _cf_descr,
                          _cf_handle);


    _safe_buffer_register(_buffer.data_ptr(),
                          _buffer.nbytes());
}
                                  ...
                                  const off_t offset)
{
    ...
    const auto error_code = IS_CUFILE_ERR(return_code)  ? cuFileGetErrorString(return_code) : cuFileGetErrorString(error_num);
    std::cerr << op_string
        << error_string
        << error_code
        << " return code = "
...
void gds_op_desc_t::_read_file(const int tid)
{
    ...
    auto ret = cuFileRead(_cf_handle,
                          data_ptr(),
                          _num_bytes_per_thread,
                          base_offset,
                          base_offset);
	if (ret < 0) {
        _report_error(ret, errno, base_offset);
	}
}
...
{
    const auto base_offset = _num_bytes_per_thread * tid;
    auto ret = cuFileWrite(_cf_handle,
                          data_ptr(),
                          _num_bytes_per_thread,
                          base_offset,
                          base_offset);
